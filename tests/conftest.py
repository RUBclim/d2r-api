...
async def clean_db(db: AsyncSession) -> AsyncGenerator[None]:
    ...
    await db.execute(delete(BiometDataHourly))
    await db.execute(delete(BiometDataDaily))
    await db.execute(delete(TempRHDataHourly))
    await db.execute(delete(TempRHDataDaily))
